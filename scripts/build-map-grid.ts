...
function main() {
  ...
  // Resolution 320×160, latMax 75°:
  //   • Each cell ≈ 1.125° lon × 0.94° lat, which keeps narrow peninsulas and
  //     medium-size states more recognizable in the dotted terminal map
  //   • The raw JSON is still small enough for this static visual asset
  const cols = 320;
  const rows = 160;
  const grid = rasterizeGrid(fc, { cols, rows, latMax: 75 });
